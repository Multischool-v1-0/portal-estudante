...
const verificationCodes = new Map<string, { code: string; expires: number; attempts: number; method?: string }>();
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'send') {
      ...
      try {
        ...
        if (method === 'whatsapp') {
          ...
          } else if (twilioError.code === 21211 || twilioError.code === 21212) {
            return NextResponse.json(
              { 
                success: false, 
                error: 'WhatsApp não está configurado corretamente. Enviando por SMS...',
                fallbackToSMS: true
              },
              { status: 400 }
            );
    ...
    if (action === 'verify') {
      ...
      if (!storedData) {
        return NextResponse.json(
          { success: false, error: 'Código não encontrado. Solicite um novo código.' },
          { status: 404 }
        );
      }
